...
public class UserServiceImpl extends AbstractServiceImpl<User, UUID, UserRepository> implements UserService {
    ...
    public EmailToken requireActivation(UserEmailDTO userEmailDTO) {
        User user = this.findByEmail(userEmailDTO.email());
        if (user.getEnabled()) {
            throw new AccountActivationException(messageSource.getMessage("messages.account.activation", null, request.getLocale()));
        }
        kafkaTemplate.send(
                KafkaConfig.TOPIC_SEND_EMAIL,
                new EmailDTO(
                        user.getEmail(),
                        messageSource.getMessage("messages.account.activation.enabled", null, request.getLocale()),
